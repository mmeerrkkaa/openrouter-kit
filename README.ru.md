...
*   `getHistoryAnalyzer(): HistoryAnalyzer`: **(Новое)** Возвращает анализатор истории.
...
*   `on(event, handler)` / `off(event, handler)`: Подписка/отписка от событий (`'error'`, `'security:*'`, и т.д.).
...
*   **Анализ истории (`HistoryAnalyzer`):**
    ...
    *   Методы принимают опциональный объект `HistoryQueryOptions` для фильтрации записей перед анализом (по дате, моделям и т.д.). См. пример ниже.
