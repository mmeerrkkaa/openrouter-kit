...
const client = new OpenRouterClient({
  ...
  model: "x-ai/grok-4-fast" // Модель по умолчанию для всех вызовов
});
...
const client = new OpenRouterClient({
  apiKey: process.env.OPENROUTER_API_KEY || "sk-or-v1-...",
  model: "x-ai/grok-4-fast", // Модель, поддерживающая инструменты
});
...
### 6. Потоковая передача ответа (Стриминг)

Получайте ответы в режиме реального времени для лучшего пользовательского опыта при генерации длинного контента.
...
const client = new OpenRouterClient({
  apiKey: process.env.OPENROUTER_API_KEY || 'sk-or-v1-...',
  model: 'x-ai/grok-4-fast',
  enableCostTracking: true, // Отслеживание стоимости даже в режиме стриминга
});
...
async function main() {
  try {
    ...
    const result = await client.chatStream({
      ...
      streamCallbacks: {
        onContent: (content) => {
          // Выводим каждый фрагмент по мере поступления
          process.stdout.write(content);
        },
        onComplete: (fullContent, usage) => {
          console.log('\n\n--- Стрим завершён ---');
          console.log('Всего токенов:', usage?.total_tokens);
        },
        onError: (error) => {
    ...
    // Результат включает полный контент, usage, стоимость и длительность
    console.log('Стоимость:', result.cost ? `$${result.cost.toFixed(6)}` : 'N/A');
    console.log('Длительность:', `${result.durationMs}ms`);
...
**Выполнение инструментов в стриминге:**

```javascript
// Инструменты выполняются автоматически при передаче (как в client.chat())
const result = await client.chatStream({
  prompt: "Какая погода в Париже?",
  tools: myTools, // 🔥 Если переданы tools, они выполнятся автоматически!
  streamCallbacks: {
    onContent: (content) => console.log(content),
    onToolCallExecuting: (name, args) => console.log(`Вызов ${name}...`),
    onToolCallResult: (name, result) => console.log(`Результат: ${result}`)
  }
});
```

**Возможности стриминга:**
- ✅ Доставка контента в реальном времени через коллбэки
- ✅ Автоматическое отслеживание стоимости и метрик
- ✅ **Автоматическое выполнение инструментов** (при передаче tools)
- ✅ Поддержка AbortSignal для отмены
- ✅ Полная проверка аутентификации и безопасности
- ✅ Управление историей с потоковыми ответами

**Примеры:**
- [`examples/streaming-demo.js`](./examples/streaming-demo.js) - Всесторонние примеры стриминга
- [`examples/streaming-test.js`](./examples/streaming-test.js) - Тестовый набор для стриминга
...
*   **🌊 Поддержка стриминга:** Потоковая передача ответов в реальном времени с `client.chatStream()` для лучшего UX. Включает коллбэки, отслеживание стоимости и полные проверки безопасности.
...
*   `model?` (string): Модель по умолчанию для запросов (например, `"x-ai/grok-4-fast"`).
...
*   `chatStream(options: OpenRouterRequestOptions, abortSignal?: AbortSignal): Promise<ChatStreamResult>`: **(Новое)** Потоковая передача ответов в реальном времени с коллбэками. Возвращает результат со стоимостью, длительностью и опциональными вызовами инструментов.
...
*   `streamCallbacks?` (StreamCallbacks): **(Только для `chatStream`)** Коллбэки для обработки событий стриминга:
    *   `onChunk?: (chunk: StreamChunk) => void`: Вызывается для каждого полученного SSE чанка.
    *   `onContent?: (content: string) => void`: Вызывается при получении нового контента.
    *   `onToolCallExecuting?: (toolName: string, args: any) => void`: Вызывается когда инструмент начинает выполняться (инструменты выполняются автоматически при передаче).
    *   `onToolCallResult?: (toolName: string, result: any) => void`: Вызывается когда инструмент завершает выполнение.
    *   `onComplete?: (fullContent: string, usage?: UsageInfo, toolCalls?: ToolCall[]) => void`: Вызывается при завершении стриминга.
    *   `onError?: (error: Error) => void`: Вызывается при ошибках стрима.
...
**`ChatStreamResult`** (возвращается `client.chatStream()`):

*   `content` (string): Полный потоковый контент.
*   `usage?` (UsageInfo | null): Статистика использования токенов.
*   `model?` (string): Модель, сгенерировавшая ответ.
*   `finishReason?` (string | null): Причина остановки стриминга.
*   `id?` (string): OpenRouter ID ответа.
*   `toolCalls?` (ToolCall[]): Вызовы инструментов, обнаруженные при стриминге (не выполняются автоматически).
*   `reasoning?` (string): Токены рассуждения, если запрошены.
*   `annotations?` (UrlCitationAnnotation[]): Цитаты веб-поиска.
*   `cost?` (number | null): Рассчитанная стоимость (если отслеживание стоимости включено).
*   `durationMs?` (number): Общая длительность стриминга в миллисекундах.
